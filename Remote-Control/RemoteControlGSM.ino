...
char SERIAL_NUMBER[] = "IUT21102017\0";
    ...
    /* phone call management routine */
          ...
          if(gsm.getCallName().length() > 1){
            ...
            if(reply){ 
              ...
              String num = gsm.getCallNumber();
              Response(num, response); 
              response = String("");
            }
          }
        while(!(error = gsm.hangoffCall())){ delay(100); }
...
*/
void outputUpdate(){
  String user_name = gsm.getNameSms(2);
  String num = gsm.getNumberSms(2);
  if((user_name != "") || ( num == superAdmin)){  
    switch(process(sms)){
      ...
      case 10: {
        ...
        digitalWrite(led_, LOW);
        _state_1 = 0;
        if(reply) { response = String("EQ1OFF"); }
        break;
      }
      case 20: {
        digitalWrite(output_2_, LOW);
       _state_2 = 0;
        if(reply) { response = String("EQ2OFF"); }
        break;
      }
      case 30: {
        digitalWrite(output_1_, LOW);
        ...
        _state_1 = 0;
        _state_2 = 0;
        if(reply) { response = String("EQOFF"); }
        break;
      }
      case 4: {
        //gsm.sendSms()
        ...
        if(_state_2){ response += String("EQ2ON"); }
        else{ response += String("EQ2OFF"); }
        break;
      }
      case 5: { // Add phone number
        if((user_name.indexOf("_adm") != -1) || ( num == superAdmin ) ){
          ...
          response = String("ADD_ERROR; Need admin credential!");
        }
        if(reply){ 
          if(ans){ response = String("ADD_OK"); } 
          else{ response = String("ADD_ERROR"); } 
        }
        break;
      }
      case 6: { // Delete phone number
        if(( user_name.indexOf("_adm") != -1 ) || ( num == superAdmin ) ){
          ...
          if(reply){ 
            if(ans){ response = String("DEL_OK"); } 
            else{ response = String("DEL_ERROR"); } 
          }
          //gsm.listNumber(1, 5);
        }else if(reply){
          response = String("ADD_ERROR; Need admin credential!");
      ...
      case 7: { // Enable or disable reply
        ...
        error = gsm.delAllSms();
        logs();
        if(reply){  response = String("REP_ON"); } 
        else{ response = String("REP_OFF"); }

        break;
      }
      case 8: {
        num.toCharArray(buffer_0, num.length());
  ...
  if(reply && (response != "")){ Response(num, response); }
...
*/
void Response(String number, String msg){
